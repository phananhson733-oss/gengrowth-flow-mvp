...
| 字段     | 值                                             |
| -------- | ---------------------------------------------- |
| 项目名称 | GenGrowth Flow MVP                             |
| 项目路径 | 当前工作目录（flow repository root）            |
...
本仓库的项目指令只来自当前仓库内的 `AGENTS.md` 及任务明确指定的项目文件。团队成员的个人档案属于个人或对应仓库上下文，不得作为 GenGrowth Flow MVP 的项目级身份、所有者声明或默认指令，也不得跨仓库回退读取。
...
| 2026-04-29 | v2.11 新增会话上下文约定（后续已收敛为项目上下文边界）                              |
| 2026-04-30 | v2.12 新增 §七 跨会话待办提醒：三模型统一读取/写入 reminders.md，修正 Gemini 角色描述（可写文件，冲突以 Claude Code 为准）|
| 2026-07-15 | v2.13 修正项目名称，移除项目级个人档案绑定，明确跨仓库个人上下文不得作为 Flow 项目指令       |
