...
4. **Schema and metadata QA.** It validates structured data against the Schema.org spec and the [[<TBD-internal-link: explainer on structured data for SEO>]] rules, so every product or article page carries the right markup at scale.
