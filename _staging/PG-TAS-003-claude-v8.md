...
A team running this audit from a generic checklist will confirm hreflang "exists" on every page and still lose rankings, because Google discards the entire annotation set when the return links do not pair up. This guide audits the way the search engine actually reads the markup.
