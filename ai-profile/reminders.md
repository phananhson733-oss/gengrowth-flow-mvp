...
- [ ] 2026-07-03 | **GSC 补提两站剩余未收录页（7/2 配额已用完，续做）**：7/2 已手动 Request Indexing 提交（经 Chrome MCP / xdawayer@gmail.com）——**astrologywiki 已提交 6 篇**：cardi-b-birth-chart、carlos-alcaraz-birth-chart、pride-month-astrology、total-solar-eclipse-2026、usa-pluto-return-astrology、ben-shelton-zodiac-sign，然后 morocco 触"超出配额"→当日配额约用 7 条到顶。**gengrowth 已提交 2 篇**：`/en/glossary/ai-content-detection`、`/en/use-cases/saas-zero-to-1000`（gengrowth 配额独立，还没用完，但搜索框聚焦不稳、UI 慢，留到下轮续做）。**已收录无需提交**（7/2 实测）：matheus-cunha、mo-salah、prince-william、jannik-sinner。**重定向跳过**：`/en/wiki/solar-return-chart`（GSC 报"网页会自动重定向"——这是内容/配置问题，值得查为何 sitemap 里的规范 URL 会重定向）。**次日续提清单**：astrologywiki 剩 `morocco-world-cup-2026-astrology`、`serena-williams-birth-chart`（+ 未验的 zh：matheus-cunha / solar-return-chart，但 solar-return 是重定向）；gengrowth 剩 8 个 glossary/use-case：`/en/glossary/topical-authority`、`/zh/glossary/robots-txt`、`/en/glossary/zero-click-search`、`/en/glossary/referral-program`、`/en/glossary/robots-txt`、`/en/glossary/ai-citation`、`/en/glossary/attribution-model`（`/glossary/referral-program` 无 locale 那条疑似重定向，检查时若显示重定向则跳过）。两站 sitemap 均已提交且成功读取（aw 426 页 / gg 191 页），不补也会被自然抓取。**操作要点（重要）**：GSC 顶部搜索框聚焦不稳——点击后**先打字再截图确认文本已进框**再回车（约半数尝试文本没落进去，需重点；提交成功后 toast 会盖住框、别马上再输入，切资源/重导航后重新 find ref）；**先确认检查结果为"未收录"再点请求，绝不盲点**（省配额，每站约 7-10/天）。注：Indexing API 对普通 blog 不合规（官方仅 JobPosting/BroadcastEvent），改走手动 UI 是 wzb 定的方向。
