...
- [ ] 2026-07-02 | **GSC 补提两站未收录页（7/1 两站配额均已耗尽，一条都没提进去）**：7/1 手动走 Request Indexing，`sc-domain:astrologywiki.com` 和 `sc-domain:gengrowth.ai` **首次尝试即"超出了配额"**（当日 ~10-12/天，可能账号级共享），两站均 0 提交。配额重置后（次日）经 Chrome MCP 在 GSC（已登录 xdawayer@gmail.com）逐条补提，提到配额上限为止。**astrologywiki（最新优先）**：cardi-b-birth-chart、carlos-alcaraz-birth-chart、matheus-cunha-birth-chart、mo-salah-birth-chart、pride-month-astrology、prince-william-birth-chart、solar-return-chart、total-solar-eclipse-2026、usa-pluto-return-astrology（均 `https://www.astrologywiki.com/en/wiki/<slug>`）+ 2 篇 zh（matheus-cunha / solar-return-chart）。**gengrowth（10 个"已抓取-尚未编入索引"页，非 blog——最新 blog 已收录）**：`/en/glossary/ai-content-detection`、`/en/use-cases/saas-zero-to-1000`、`/en/glossary/topical-authority`、`/zh/glossary/robots-txt`、`/en/glossary/zero-click-search`、`/en/glossary/referral-program`、`/en/glossary/robots-txt`、`/en/glossary/ai-citation`、`/en/glossary/attribution-model`（`/glossary/referral-program` 无 locale 那条疑似重定向，检查时若显示重定向则跳过）。若配额有限，先提 astrologywiki 最新文章。两站 sitemap 均已提交且 7/1 成功读取（aw 426 页 / gg 191 页），不补也会被自然抓取。操作要点：搜索框中性点击+等出下拉再输入；**先确认检查结果为"未收录"再点请求，绝不盲点**（省配额）。注：Indexing API 对普通 blog 不合规（官方仅 JobPosting/BroadcastEvent），改走手动 UI 是 wzb 定的方向。
