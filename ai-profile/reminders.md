...
- [ ] 2026-07-02 | **GSC 补提 astrologywiki 最新文章（7/1 配额已耗尽）**：7/1 手动走 Request Indexing 时 sc-domain:astrologywiki.com 当日配额(~10-12/天)已被用光，`cardi-b-birth-chart` 起一条都没提交进去（"超出了配额"）。配额重置后（次日）经 Chrome MCP 在 GSC（已登录 xdawayer@gmail.com / sc-domain:astrologywiki.com）从最新文章开始逐条补提，提到配额上限为止：cardi-b-birth-chart、carlos-alcaraz-birth-chart、matheus-cunha-birth-chart、mo-salah-birth-chart、pride-month-astrology、prince-william-birth-chart、solar-return-chart、total-solar-eclipse-2026、usa-pluto-return-astrology（均 `https://www.astrologywiki.com/en/wiki/<slug>`）+ 2 篇 zh（matheus-cunha / solar-return-chart）。均已在 sitemap（7/1 成功读取 426 页），不补也会被自然抓取。操作要点：搜索框聚焦用中性点击+double_click+等出下拉再输入；**先确认 URL 检查结果为"未收录"再点请求，绝不盲点**（省配额）。注：Indexing API 方式对普通 blog 不合规（官方仅 JobPosting/BroadcastEvent），改走手动 UGC UI 是 wzb 定的方向。gengrowth 的 10 个未收录页已在 7/1 提交。
