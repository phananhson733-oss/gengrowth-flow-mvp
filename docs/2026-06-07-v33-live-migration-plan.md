...
- [x] **cluster_id 自动同步 DONE（2026-06-09，D4=自动同步+漏检报告）**：`gg-cluster-sync.mjs`（精确唯一命中自动填 AC，additive/幂等/不覆盖；冲突/模糊/孤儿只报告）接入 `gg-seo-autopilot-tick.sh`（每次 fire 跑 `--apply`，独立日志 `cluster-sync-YYYY-MM-DD.log`，NON-FATAL 不阻断内容线）。6 smoke 测试 + 全量 1050 绿。commit 386b3df。**漏检报告堵两种漏**：① 疑似漏配（仅模糊命中，如 `ketu mahadasha~vedic_mahadashas`）② 集群表对不上主表的孤儿词（措辞/拼写漂移，如 `houses_life_areas:"9th house"` 主表是 "9th house astrology"、带 "(NEW)" 后缀的录入瑕疵）。线上现状 synced=0(41已填) fuzzy=9 orphans=27 unclustered=413。**运营动作**：定期看 cluster-sync 日志，把疑似漏配/孤儿词回写进 keywords_included（措辞对齐主表），自动同步即接住。
