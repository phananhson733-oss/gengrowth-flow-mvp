...
|---|---|---|---|---|---|---|---|
| ⚙️配置 | 2 列 + 多区段 | 2 列 + rows | ✅ | n/a | n/a | n/a | ⚠️ `headerColors` 字段拼写不匹配（spec 写 `headerColors`，builder 只读 `headerColorByCol`），A1/B1 深灰不会上色（cosmetic） |
...
公式列 K2/O2 都引用 `'⚙️配置'!$A$6:$A$25` 和 `$A$28:$A$45`，区域命名稳定。
