---
title: GenGrowth MVP 落地 plan v1.1.3（lean2.1 + autoplan R3 architectural land + RACI v1 incorporate）
date: 2026-05-20
updated: 2026-05-21
...
companion_doc:
  ...
  - G-GenGrowth-MVP-RACI-and-execution-flow-v1.md (RACI v1)
...
revision_history:
  ...
  - version: v1.1.3
    date: 2026-05-21
    summary: |
      incorporated RACI v1 §3 step-level matrix (30 steps × 8 cols) 作为 §0.3；
      W0/W1 Mon 7 个新工具 ship status 更新（keyword-fallback / day1-gate-check / facts-audit / entity-passport / friction-mine / gate-check / sop-draft 全 ship + 各自 spec + smoke）；
      Tech v1.2-lean2.1+patch + PRD v1.3 三档对齐 G4/G14。
      Day-0 GSC baseline 实测 ≈ 250 imp/30d，触发 W1 keyword fallback default path。
tags:
  - gengrowth
  - mvp
  - landing-plan
  ...
  - quality-first
  - geo
  - raci-v1-incorporated
aliases:
  - GenGrowth 落地 plan
  - landing plan v1.1
  - landing plan v1.1.2
  - GenGrowth 落地 plan v1.1.3
  - lean2.1 校准 patch
  - autoplan R3 architectural land
  - 5 周执行计划
---

# GenGrowth MVP 落地 plan v1.1.3（lean2.1 + RACI v1 incorporate）

> [!danger] DEPRECATED 2026-05-21 — 整份文档 SUPERSEDED
> 本文档（plan v1.x derivative line + RACI v1 §0.3 incorporated）是企业级 over-engineering，**对 1-2 人小团队不适用**。
> 真正 canonical 文档是 Lynne 已写好的 3 份：
> - `docs/03-marketing/03-seo/keyword-research-sop.md` v2.5（六源 + 四桶 + 三关过滤）
> - `docs/03-marketing/03-seo/keyword-sheet-setup.gs` v3.1（24 列关键词主表 + 6-ID 体系）
> - `docs/03-marketing/2026-05-15-gengrowth-internal-growth-mvp-prd-v0.7.md`（量产线/精修线 + 主题集群）
>
...
>
> **配套阅读**：[[G-GenGrowth-MVP-OpsPM-PRD-v1.2-lean]]（PRD，业务目标）+ [[G-GenGrowth-MVP-半自动化工具栈方案-v1.2-lean]]（技术细节）+ [[G-GenGrowth-MVP-RACI-and-execution-flow-v1]]（RACI 评审 + 18 gap + 50 decision 压缩）。
>
...
## §0.3 RACI step-level matrix（incorporated 自 RACI v1 §3）

> [!info] 本段定位
> 30 steps × 8 列（Step ID / step / Trigger / R / A / C / I / Output / Sign-off owner / Fallback），
> 是 wzb "只看 + 填" 角色的 step-level 落地表。
> 完整 RACI 评审 + 18 gap + 50 decision 压缩详见 [[G-GenGrowth-MVP-RACI-and-execution-flow-v1]]。
...
**RACI 关键规则**：
- R = 责任执行（who does the work）
- A = 问责验收（who signs off）
- C = 咨询（who advises）
- I = 告知（who is informed）
- Sign-off owner ≠ A（Sign-off 是动作；A 是身份）
- Fallback 永远存在（不允许 step 无降级路径）

**改造收益 4 维**（详见 RACI v1 §0 TL;DR）：
1. 5 周累计 wzb 时间省 ~8-12h
2. surface count 4 周红色 → 0 周红色
3. wzb micro-decision 80 → 30 次
4. ROI ≈ 2.5x

---

## §0.4 W0/W1 Mon 工具 ship status（2026-05-21）

| 工具 | 优先级 | Spec | 实现 | smoke | 备注 |
|------|--------|------|------|-------|------|
| `bin/gg-day1-gate-check` | P0-3 | [[G-GenGrowth-MVP-day1-gate-check-tool-spec-v1]] | ✅ ship | ✅ 19/19 | 5 binary gate（含 Lynne sign-off）|
| `bin/gg-facts-audit` | P0-1 | [[G-GenGrowth-MVP-facts-audit-tool-spec-v1]] | ✅ ship | ✅ 32/32 | 5 断言（regex AST + Sheets + GSC PII grep + severity）|
| `/gg-keyword-fallback` | P1-1 | [[G-GenGrowth-MVP-keyword-fallback-tool-spec-v1]] | ✅ ship | ✅ 49/49 | zero-baseline default path（GSC empty 实测触发，Reddit+SERP 双源）|
| `/gg-entity-passport` | P1-1 | [[G-GenGrowth-MVP-entity-passport-tool-spec-v1]] | ✅ ship | ✅ 55/55 | 5 源（Wikipedia/astro.com/cafeastrology/chaninicholas/reddit）× 5 角度 |
| `/gg-friction-mine` | P1-1 | [[G-GenGrowth-MVP-friction-mine-tool-spec-v1]] | ✅ ship | ✅ 45/45 | Reddit-only（Quora 砍）+ AI 抽 3-5 friction 点 |
| `bin/gg-gate-check` | P1-2 | [[G-GenGrowth-MVP-gate-check-tool-spec-v1]] | ✅ ship | ✅ 58/58 | 6 gate dispatcher（day-1 forward + weekly/vertical-slice/day-14/30/60）|
| `/gg-sop-draft` | P1-3 | [[G-GenGrowth-MVP-sop-draft-tool-spec-v1]] | ✅ ship | ✅ 18/18 | 5 SOP 模板（m9/monday/reddit/ai-monitor/social-distribute）+ 5 份 draft 实跑落地 |
| `bin/gg-bootstrap-gcp` | P0-2 | — | ❌ 推迟 | — | wzb 已手工建 GCP，工具仅"下次换 SA 省 1h"边际价值 |

**累计**：7 个工具 ship + 8 个 spec + 7 套 smoke 共 **276 tests pass** + 5 份 Ops SOP draft 已落地。

**未 ship（后续 W2+）**：
- W2: `/gg-content-draft-minimal` + `bin/publish-backfill --confirm`
- W3: `bin/event-export` 提前到 W3 + AI cluster proposer
- W4: `bin/gg-ai-citation-check` + `bin/gg-day14-check`
- W5: `bin/gg-retro-pack` + `bin/gg-acceptance-check` + seo-gate-scan 分级
...
- [ ] DataForSEO 账号已正式登录 + API key 可用（lean2 已确认，无需 KYC 等待）
...
- [ ] **test sheet 已创建 + reader/writer SA 可读写验证（手工跑一次 Sheets API ping）**
- [ ] Ops 候选名字 + 联系方式 + 工时合同 → 记入本文 §9 凭据登记表
- [ ] **Ops backup person 名单**（至少 1 人）→ 记入本文 §9
- [ ] 5 篇精选决策表 → 记入本文 §9
- [ ] **Lynne Day 30/60 kill 投票权 sign-off**（v1.1.2 新增）→ 记入本文 §9 + Lynne 邮件/IM 确认存档
- [ ] Anthropic Claude API key 可用（已有）
- [ ] OpenAI Codex API key 可用（T1 跨模型挑战用）
- [ ] oracle repo 在本机可 PR + merge（M9 git mechanics 前置）
...
| [[G-GenGrowth-MVP-RACI-and-execution-flow-v1]] | RACI 评审 / 18 gap / 50 decision 压缩 / step-level matrix 完整源 |
...
| **v1.1.3** | **2026-05-21** | **RACI v1 §3 step-level matrix incorporate + W0/W1 Mon 7 工具 ship status 更新**。本次修：(1) 新增 §0.3 RACI step-level matrix（30 steps × 8 列：Step ID / step / Trigger / R / A / C / I / Output / Sign-off owner / Fallback），完整从 [[G-GenGrowth-MVP-RACI-and-execution-flow-v1]] §3 复制，全表 30 行；配 RACI 关键规则 + 改造收益 4 维（5 周省 ~8-12h / surface count 4→0 红 / wzb micro-decision 80→30 / ROI 2.5x）。(2) 新增 §0.4 W0/W1 Mon 工具 ship status 表：7 工具 ship（day1-gate-check P0-3 + facts-audit P0-1 + keyword-fallback P1-1 + entity-passport P1-1 + friction-mine P1-1 + gate-check P1-2 + sop-draft P1-3）+ 8 spec + 7 套 smoke 共 **276 tests pass** + 5 份 Ops SOP draft 已落地；`bin/gg-bootstrap-gcp` 推迟（wzb 已手工建 GCP）。未 ship roadmap：W2 content-draft-minimal + publish-backfill --confirm / W3 event-export 提前 + AI cluster proposer / W4 ai-citation-check + day14-check / W5 retro-pack + acceptance-check + seo-gate-scan 分级。(3) §0.3 S-W2-2 row 标注 "GSC empty baseline（2026-05-21 已验证）→ `/gg-keyword-fallback` default path"——Day-0 GSC baseline 实测 ≈ 250 imp/30d，触发 W1 keyword fallback default path（zero-baseline 已成默认路径，不再是 fallback 边缘 case）。(4) §12 关联文档加入 RACI v1 entry。(5) Tech v1.2-lean2.1+patch + PRD v1.3 三档对齐 G4/G14 已同步（参 RACI v1 配套）。**未动**：§1-§12 任何已有内容（除 §12 加 RACI v1 link 一行）；Tech / PRD / RACI 文档本身；`tools/scripts/` 代码；Day 60 KPI；Day 60 judge = Lynne。|

---

**一句话结尾（v1.1.3）**：这份是 lean2 双档的**执行映射表 + 现实性校准 + 三轮 autoplan adversarial review 的 architectural land + RACI v1 step-level incorporate + W0/W1 Mon 7 工具 ship status 实测**。每一条 `[ ]` 都来自 PRD/Tech/RACI 决策，经 autoplan 3-voice × 3 轮复审做执行诚实性修正 + architectural 砍载 + RACI step-level "只看+填" 角色对齐。**勾掉一条就近一步**，不勾就是阻塞。Day-1 18:00 真 binary gate（5/5 必满足，含 Lynne sign-off）是分叉点——过 → 主线；任一缺 → §1B solo-fallback。**今天就动 Day-0 4 件事**（Ops + GCP + 5 篇决策 + Lynne sign-off）。GSC empty baseline 已实测（≈250 imp/30d），W1 keyword fallback default path 已激活。

— landing plan v1.1.3 / RACI v1 §3 incorporated / W0+W1 Mon 7 ship status / 2026-05-21
