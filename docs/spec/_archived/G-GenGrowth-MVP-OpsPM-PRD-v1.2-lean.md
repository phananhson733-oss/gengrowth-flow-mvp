---
title: GenGrowth 内部增长 MVP — Ops/PM 产品需求文档（PRD）v1.2-lean2
date: 2026-05-20
updated: 2026-05-20
type: ops-pm-prd
...
# GenGrowth 内部增长 MVP — Ops/PM 产品需求文档（PRD）v1.2-lean2

> [!danger] DEPRECATED 2026-05-21 — 整份文档 SUPERSEDED
> 本文档及其后继版本 v1.3 都是基于 RACI v1 的企业级 over-engineering，**没读上游 Lynne PRD v0.7 就改了 schema**，方向跑偏。
...
| **Ops 中途退出**（lean2 新增）| wzb 接所有运营 → +5-8h/周 | wzb 红线放宽到 24h 应急；工具线砍到必要 3 项（content-draft + cta-inject + publish-backfill）|
...
| **3 篇精修发布后 0 AI 引用**（lean2 GEO 取向） | Day 7-14 监测 | wzb retro 调整精修 SOP（可能改 Entity Passport 取证深度 / Friction 选材角度）|
| Day 30 PV < 5%（精修线达不到 Top 50）| 复盘表 | sunset 该 cluster 或升级到 T1 Pillar 重写 |
| **Ops 退出**（lean2 新增）| Ops 通知中途退出 | wzb 红线临时放宽到 24h；工具线砍到 3 项；Week-5 不延期 |
...
| 文档 | 用途 |
|------|------|
| **本文（v1.2-lean Ops/PM PRD）** | 你正在看 |
| **v1.2-lean 技术版** | 工程实施细节、数据表 schema、API 调用、测试矩阵 |
| [[G-GenGrowth-MVP-落地plan-v1.1]] | **执行 checklist v1.1.2**（lean2.1 校准 + autoplan R3 architectural land：3 轮 adversarial review 落地）。把 PRD/Tech 决策翻译为可点 todo + 现实性砍载（W1 ~16h / 9-12 篇精修）+ Day 30/60 Lynne judge + Day-1 真 binary 5/5 + §1B solo-fallback plan。**v1.1.2 是当前 canonical 执行档**；本 PRD lean2 prose 中的 "12-15 篇" / "20h Week-4" / "wzb 自决 Day 60" 是 lean2 历史快照。|
...
**一句话结尾（lean2）**：5 周 / wzb 18h/周 + Ops 5-8h/周 / $100 月度预算内，把 SEO 内容生产线从全手工变成**精修取向半自动**，目标 Day 60 拿 **≥3 篇 Top 10 + ≥3 篇 AI Overview / Perplexity 引用**。GEO 时代质量 >> 数量。半自动定位永远不漂移到全自动，wzb 永远是 T1/T2 最后审稿人。

— v1.2-lean2 / 2026-05-20 深夜
