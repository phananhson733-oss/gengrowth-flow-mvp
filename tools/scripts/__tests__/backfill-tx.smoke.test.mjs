...
test('verify-live 未过 → deferred，留队 attempts=1，绝不写 Sheet', async () => {
  ...
  const r = await backfillOnLive(
    ...
    okDeps({ verifyLive: async () => false, flipRowsByPageId: async () => { flipped++; } }),
  );
...
test('drainPending：超 MAX_ATTEMPTS 的毒记录淘汰 → 移入 dropped/ 保留(不静默删)、不再重扫', async () => {
  const state = freshState();
  enqueueWriteback({ pageId: 'PG-DR-1', slug: 'x', site: 'astrologywiki', attempts: 8, done: ['sheet', 'plan'] });
  const out = await drainPending(okDeps());
  assert.equal(out.dropped.length, 1);
  assert.equal(out.dropped[0].pageId, 'PG-DR-1');
  assert.deepEqual(out.dropped[0].stuck, ['archive']); // 卡在 archive
  assert.equal(readWriteback('PG-DR-1'), null); // 队列顶层已移除
  // 但记录被保留在 dropped/（可审计/人工补救），不是静默删除
  assert.ok(existsSync(join(state, 'pending-writeback', 'dropped', 'PG-DR-1.json')));
