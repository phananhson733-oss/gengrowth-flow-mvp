...
# live posts are skipped. Reads a dedicated backend secret from macOS Keychain, with a
# temporary legacy service_role CLI fallback. Fail-safe: missing auth / no work / errors
# never crash — just log + alert.
#
...
SB_KEY="$(/usr/bin/security find-generic-password -a "$SB_KEYCHAIN_ACCOUNT" -s "$SB_KEYCHAIN_SERVICE" -w 2>/dev/null || true)"
if [ -z "$SB_KEY" ]; then
...
if [ -z "$SB_KEY" ]; then
  echo "$(date '+%F %T') backend secret unavailable (Keychain + legacy CLI lookup empty) — skipping this tick" >> "$LOG"
  # 统一通知事件层（NOTIFY-CONTRACT.md）：模板与 @ 策略（auth_missing → OPS）由事件表决定，
  # 不再在调用点散装 AT env / 裸拼字符串。gg-notify exit 永远 0，best-effort。
  node "$SCRIPT_DIR/gg-notify.mjs" auth_missing --site gengrowth --what backend_secret --hint "configure Supabase backend secret" >/dev/null 2>&1 || true
  exit 0
fi
export SB_KEY
