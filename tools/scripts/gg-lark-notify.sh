...
# @-mention policy (user decision 2026-06-08). A plain "@名字" in text does NOT ping —
# Feishu needs the <at user_id="..."> tag, which this adds. 王玲 is NEVER @'d. Flags:
#   GG_LARK_NOTIFY_AT_OPERATOR=1 → @王志彪  (action-needed alerts: park / gate-fail)
#   GG_LARK_NOTIFY_AT_PM=1       → @马博洋  (a day's content is ALL complete)
# Routine "已发布" notices set neither → plain text, no @.
...
OPERATOR_OID="ou_180b4310d5dcc8752bcc06115626073d"  # 王志彪 — @'d on alerts
PM_OID="ou_c2eb25026dd20d6e9fbf404a1aa49c39"        # 马博洋 — @'d on day-complete
# (王玲 ou_0c080154… is intentionally never @'d)
...
AT=""
[ "$GG_LARK_NOTIFY_AT_OPERATOR" = "1" ] && AT="${AT}<at user_id=\"$OPERATOR_OID\"></at> "
[ "$GG_LARK_NOTIFY_AT_PM" = "1" ] && AT="${AT}<at user_id=\"$PM_OID\"></at> "
