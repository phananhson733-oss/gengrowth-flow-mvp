...
export const TABS = [
...

  {
    ...
    extras: {
      ...
      notes: {
        ...
        O1: 'cta_primary：历史 CTA 意图提示（如工具页 / 注册）。主 CTA 由 CTA Map 的关键词、资格和优先级语义选择；blog 内链仍独立维护。',
...

  {
    ...
    extras: {
      ...
      notes: {
        ...
        K1: 'CTA：可选的 CTA Map cta_id 或完整 URL；只有命中当前产品的合格 CTA 记录才会覆盖语义选择。旧“工具页/星盘页”仅表示工具意图。',
...

  {
    name: 'CTA Map',
    type: 'standard',
    header: ['cta_id', 'page_role', 'cta_文案', 'target_url', 'ga4_event_name', 'track', 'desc', 'cta_kind', 'match_keywords', 'blog_eligible', 'priority', 'intent_tags'],
    headerColorByCol: { 1: 'green', 2: 'slate', 3: 'green', 4: 'green', 5: 'slate', 6: 'slate', 7: 'green', 8: 'green', 9: 'green', 10: 'green', 11: 'green', 12: 'green' },
    seedRows: [
    ...
    extras: {
      ...
      headerBold: true,
      headerFontSize: 11,
      columnWidths: [160, 100, 220, 240, 150, 90, 300, 120, 260, 110, 90, 180],
      dataValidations: [
        { range: 'B2:B1500', list: ['Pillar', 'Series', 'Support', 'Tool', 'Wiki', 'Strategic'] },
        { range: 'F2:F1500', list: ['量产线', '精修线'] },
        { range: 'H2:H1500', list: ['tool', 'feature', 'hub', 'product', 'navigation', 'blog', 'external'] },
        { range: 'J2:J1500', list: ['TRUE', 'FALSE'] },
      ],
      notes: {
        A1: 'cta_id：稳定主键。选题登记表可显式引用它；同一 ID 不可重复。',
        B1: 'page_role：历史兼容/运营分类字段；主 CTA 不再按 page_role + track 直接匹配。',
        C1: 'cta_文案：用户在页面看到的 CTA 按钮/链接文字。简短行动指令。',
        D1: 'target_url：点击 CTA 跳转的 HTTPS URL，必须属于当前产品域名；占位 URL 不可启用。',
        E1: 'ga4_event_name：已配置的 GA4 事件名；未配置时留空，不虚构事件。',
        F1: 'track：历史兼容/运营分类字段，不参与语义主 CTA 的强制匹配。',
        G1: 'desc：CTA 的用途和适用读者/意图，用于人工审阅与语义选择审计。',
        H1: 'cta_kind：tool / feature / hub / product / navigation / blog / external。blog、navigation、external 永不作为主 CTA。',
        I1: 'match_keywords：以分号分隔的英文关键词或短语。每个工作簿仅允许一个已启用的 * 通配兜底。',
        J1: 'blog_eligible：TRUE 才能作为 SEO blog 主 CTA；Blog_Article 内链必须为 FALSE。',
        K1: 'priority：语义得分相同时的正整数优先级；仍相同则按 cta_id 稳定排序。',
        L1: 'intent_tags：分号分隔的候选意图类别；选择器先按目标关键词、实体和内容角度识别标签，再在同标签候选中选择。',
      },
    },
  },
  ...
  {
    name: REQUEST_INDEXING_QUEUE_TAB,
    type: 'standard',
    header: [...REQUEST_INDEXING_QUEUE_HEADER],
    headerColorByCol: {},
    extras: {
      frozenRows: 1,
      headerBgAll: 'header',
      ...
      headerBold: true,
      columnWidths: [
        150, 80, 120, 280, 220, 90, 220, 150, 130, 120,
        120, 130, 140, 320, 300, 320, 130, 360, 110, 110, 240,
      ],
    },
  },
