...
- **`### H3` 仅用于"超长叙述章节"内部分组**（2026-06-09）：当某个叙述型章节（如 Why It Matters / vs Adjacent Concepts / How to Read）的正文 prose 会达到 4+ 段时，用 **2-3 个 `### ` 小标题**把它拆成每组 ≤3 段（对齐 SC3c），而不是堆成一面文字墙、也不是把叙述硬塞进编号列表。**H3 非必须**——短章节、或本就用「引子+编号列表」的章节，不要加 H3。**0 个 `#### H4`** — 不要更深层级。
...
- **结构硬规则不被覆盖**：表格行 / 列表项 / 标题 / 引用块不算 prose 段；**速查表 / Quick Reference section**（无论标题怎么变体）标题后第一个非空行**仍必须直接是表格**、「Reflection Prompts」标题后第一个非空行**仍必须直接是 `1.` 编号项**。
