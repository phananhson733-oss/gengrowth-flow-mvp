...
2c. **🔴 竞品事实硬要求（仅当本文比较第三方产品时生效 —— `{竞品} alternatives` / `{竞品} review` / `A vs B` / `best-X` 词族）**：
   ...
   - **🔴 写自家产品的话，责任比写竞品更重**（广告合规上对自己的断言举证责任最高，且被竞品反引最容易）：
     - 落地页/CTA 指向的页面，**写之前必须实际打开读一遍**，不能凭印象。
       实测翻车两次：写「无需登录」而该页写着 `A verified Supabase session is required to run`；
       写「免费层」而该页只指向 `/pricing`。
     - **引用自家产品页的能力声明要逐字，不许换动词**。该页写的是
       `does not **claim** GSC, traffic, or ranking data`，写成 `does not **use**` 就是更强的断言，
       而且与产品实际拉取第三方搜索数据的行为冲突。同一处措辞在两篇文章里连错两次。
     - **不要为了衬托自己而断言「整个品类都没有 X」**。实测：写「这个品类没有工具审计自己的产出」，
       而三家竞品都有审计功能，其中两个还是同一篇文章上一段自己列出来的。
       这是「虚假陈述 + 贬损竞品 + 抬高自己」三合一，是最危险的一类句子。
       要写差异，就写**范围**差异（「它们的审计只覆盖自己生成的内容」），不写有无。
