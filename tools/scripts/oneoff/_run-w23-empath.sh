#!/bin/bash
# W23 EMPATH cluster: highly sensitive person pillar + 3 spokes (signs / vs-autism
...
# output -> _staging/. Logs to /tmp/w23-empath.log. Mirrors _run-w22-transits.sh.
set -u
cd /Users/wzb/gengrowth-flow-mvp
LOG=/tmp/w23-empath.log
: > "$LOG"
BATCH=".gg-cache/batches/w23-empath.json"
OVR=".gg-cache/overrides/w23-empath.json"

# page_id|row|langs
JOBS=(
  "PG-EMPATH-001|1|en,zh"
  "PG-EMPATH-004|2|en,zh"
  "PG-EMPATH-003|3|en,zh"
  "PG-EMPATH-002|4|en,zh"
)
...
echo "W23-EMPATH BATCH START $(date)" | tee -a "$LOG"
: > /tmp/w23-empath-results.txt
for job in "${JOBS[@]}"; do
  IFS='|' read -r pid row langs <<< "$job"
  IFS=',' read -ra LS <<< "$langs"
  for L in "${LS[@]}"; do
    run_lang "$pid" "$row" "$L" | tee -a /tmp/w23-empath-results.txt
  done
done
echo "W23-EMPATH BATCH DONE $(date)" | tee -a "$LOG"
